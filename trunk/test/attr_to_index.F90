!> @brief Tests attr_to_index subroutine
!> @date Sep 2023
program test_attr_to_index
!
   use mpi_param
   use mpi_wrapper
   use physics
...
!..this test is set up for sequential computation
   if (NUM_PROCS > 1) then
      write(*,*) 'test_attr_to_index: NUM_PROCS>1'
      NPASS = 1; goto 99
   endif
!
   NPASS = 1
...
!
   ...
   if (index .ne. 1) NPASS = 0
!
   attr = 1; comp = 2;
   call attr_to_index(attr,comp, index)
   if (index .ne. 2) NPASS = 0
!
   attr = 2; comp = 1;
   call attr_to_index(attr,comp, index)
   if (index .ne. 3) NPASS = 0
!
   attr = 2; comp = 2;
   call attr_to_index(attr,comp, index)
   if (index .ne. 4) NPASS = 0
!
   attr = 3; comp = 1;
   call attr_to_index(attr,comp, index)
   if (index .ne. 5) NPASS = 0
!
   attr = 3; comp = 2;
   call attr_to_index(attr,comp, index)
   if (index .ne. 6) NPASS = 0
!
   attr = 4; comp = 1;
   call attr_to_index(attr,comp, index)
   if (index .ne. 7) NPASS = 0
!
   attr = 4; comp = 2;
   call attr_to_index(attr,comp, index)
   if (index .ne. 8) NPASS = 0
!
 99 continue
!
...
!
end program test_attr_to_index
!
!----------------------------------------------------------------------
