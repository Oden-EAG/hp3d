...
!
   call hexa_adapt_h_aniso
   if (NPASS.eq.1 .and. RANK.eq.ROOT) then
      write(*,*) 'test_hexa_adapt_h_aniso PASSED.'
   elseif (RANK.eq.ROOT) then
      write(*,*) 'test_hexa_adapt_h_aniso FAILED.'
   endif
!
...
!
      NPASS = 1
!
      mdle_ref = 0; kref_ref = 0; icnt = 0
!
      if (iprint.ge.1 .and. RANK.eq.ROOT) then
         write(*,110) ' NITR,PERC = ',NITR,PERC
     110 format(' hexa_adapt_h_aniso: ',A,I3,',',F5.2)
      endif
!
      do itr=1,NITR
!
        ...
        120 format(' hexa_adapt_h_aniso: ',A,I3,',',I5)
         endif
!
         ! iterate nref times
         nel = NRELES
...
!              ...pick one mdle from list
                  ...
                  mdle = ELEM_ORDER(idx)
!              ...pick random anisotropic refinement flag
                  call random_number(x)
                  if (x .lt. 0.33d0) then
                     kref = 1
                  elseif (x .lt. 0.66d0) then
                     kref = 10
                  else
                     kref = 100
                  endif
...
!
            if (NUM_PROCS.gt.1) then
               ...
               call MPI_BCAST(kref,1,MPI_INTEGER,ROOT,MPI_COMM_WORLD, ierr)
            endif
!
!        ...keep track of refinements
            ...
            if (icnt.le.NREF_MAX) then
              ...
              130 format(' hexa_adapt_h_aniso: ',A,I5,',',I5,',',I5)
               endif
               exit
            endif
!
           ...
           140 format(' hexa_adapt_h_aniso: ',A,I6,',',I4)
            endif
!
...
!                 (instead of ending computation with error msg)
            ...
            if (NPASS.ne.1) then
               ...
               if (RANK.eq.ROOT) then
                  write(*,150) '      icnt = ',icnt
                  do idx=1,icnt
                     write(*,160) ' mdle,kref = ',mdle_ref(idx),kref_ref(idx)
                  enddo
              150 format(' hexa_adapt_h_aniso: ',A,I6)
              160 format(' hexa_adapt_h_aniso: ',A,I6,',',I4)
               endif
               return
            endif
!        ...repartition
...
!  ...end loop over itr
      enddo
!
   end subroutine hexa_adapt_h_aniso
!
!
end program test_hexa_adapt_h_aniso
!
!
!----------------------------------------------------------------------
