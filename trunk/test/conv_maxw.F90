!> @brief Tests convergence rate for Maxwell Galerkin problem
!> @details Tests order p convergence rate of the H(curl) error
!!          for the variational curl-curl Maxwell problem
!!          with h-uniform refinements for a known solution
!> @date Mar 2023
program test_conv_maxw
!
   use control
...
!..this test is set up for sequential computation
   if (NUM_PROCS > 1) then
      write(*,*) 'test_conv_maxw: NUM_PROCS>1'
      NPASS = 1; goto 99
   endif
!
   call conv_maxw(nref)
   if (NPASS.eq.1 .and. RANK.eq.ROOT) then
      write(*,*) 'test_conv_maxw PASSED.'
   elseif (RANK.eq.ROOT) then
      write(*,*) 'test_conv_maxw FAILED.'
   endif
!
 99 continue
!
...
!
!----------------------------------------------------------------------
! !> @brief Computes convergence rate for Maxwell Galerkin problem
! !> @date Mar 2023
!----------------------------------------------------------------------
   subroutine conv_maxw(Nref)
!
...
!
         if (rate < 0.90d0 .or. rate > 1.10d0) then
            NPASS = 0
            iprint = 1
         endif
!
         if (iprint.eq.1) then
            ...
            write(*,212) rate
        210 format(/,'conv_maxw: dof_last, err_last = ',I10,' ,',ES12.4)
...
!
      enddo
!
   end subroutine conv_maxw
!
!
end program test_conv_maxw
!
#include "typedefs.h"
!
!----------------------------------------------------------------------
...
!
   if (NRINDEX .ne. 1) then
      write(*,*) 'conv_maxw: NRINDEX = ',NRINDEX
      stop
   endif
!
   do iel=1,NRELIS
