!> Purpose - return nodal connectivity
...
!
  ...
  integer, dimension(27), intent(inout) :: Nodesl,Norientl

  ! history information nfathl, isonl
  integer, dimension(MAXGEN) :: nfathl, isonl
  integer, dimension(27)     :: nodesl_fath,norientl_fath
  integer :: iprint, igen, nfath, nod, nson, nrgen, n_nodes,nrsons
  !
!-----------------------------------------------------------------------
!
  select case(Mdle)
...
!
  ...
  ! initialize
  Nodesl = 0; Norientl = 0; nodesl_fath = 0; norientl_fath = 0;
...
7011    format('igen = ',i2,' father = ',i6,' son number = ',i2)
     enddo
  endif
  !-------------------------------------------------------------
