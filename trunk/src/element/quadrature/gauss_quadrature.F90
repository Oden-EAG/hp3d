...
!
!======================================================================
!  Constants for normalized integrated Legendre polynomials           |
!======================================================================
!
      real(8), dimension(9,9) :: XLEG0
!$OMP THREADPRIVATE (XLEG0)
!
!
!======================================================================
!  Integration points and corresponding weights for Gaussian          |
!  integration on interal [-1,1].                                     |
!  Remark: they are rescaled to interval [0,1] by routine             |
!  init_gauss_quadrature                                              |
!======================================================================
!
      real(8), dimension(10,10) :: XIGAUS1
!$OMP THREADPRIVATE (XIGAUS1)
      real(8), dimension(10,10) :: WAGAUS1
!$OMP THREADPRIVATE (WAGAUS1)
!
!
!======================================================================
...
!$OMP THREADPRIVATE (XIGAUSJ)
      real(8), dimension(10,10) :: WAGAUSJ
!$OMP THREADPRIVATE (WAGAUSJ)
!
!
!======================================================================
!  Integration points and corresponding weights for 2-D Gaussian      |
!  integration on a triangle                                          |
!                                                                     |
!  XIGAUSS(i,j,k) = i-th affine coordinate of j-th point of k-th      |
!                   points set                                        |
!  WAGAUSS(j,k)   = weight at j-th point of k-th points set           |
!  NRGAUPO(k)     = number of Gauss points in the k-th points set     |
!  NSELECT(k)     = number of points set to integrate exactly         |
!                   mass matrix for an affine element and order k     |
!======================================================================
!
      real(8), dimension(3,79,20) :: XIGAUSS
!$OMP THREADPRIVATE (XIGAUSS)
      real(8), dimension(  79,20) :: WAGAUSS
!$OMP THREADPRIVATE (WAGAUSS)
!
...
!
!======================================================================
!  Integration points and corresponding weights for 3-D Gaussian      |
!  integration on the master tetrahedron                              |
!                                                                     |
!  XIGAUSS_TETS(i,j,k) = i-th coordinate of j-th point of k-th points |
!                        set                                          |
!  WAGAUSS_TETS(j,k)   = weight at j-th points of k-th point set      |
!  NRGAUPO_TETS(k)     = number of Gauss points in the k-th set       |
!  NSELECT_TETS(k)     = number of integration rule to integrate      |
!                        exactly mass matrix for k-th order of        |
!                        approximation                                |
!======================================================================
!
      real(8), dimension(3,720,7) :: XIGAUSS_TETS
!$OMP THREADPRIVATE (XIGAUSS_TETS)
      real(8), dimension(  720,7) :: WAGAUSS_TETS
!$OMP THREADPRIVATE (WAGAUSS_TETS)
!
...
!
!  ...initialize workspaces [Yes, should have been declared as
!     parameters...]
      ...
      /.046875697427642d0, .333333333333333d0, .333333333333333d0, &
       ...
       .900399064086661d0/
!      data((barcoo(k,j,17),k=1,4),j=1,15)
...
!       .655493203809423d0, .026805742283163d0, .135440871671036d0,
       .655493203809423d0/
...
!      data((barcoo(k,j,18),k=1,4),j=1,17)
...
!       .043146367216965d0, .006331914076406d0, .008430536202420d0,
       .043146367216965d0/
...
!      data((barcoo(k,j,19),k=1,4),j=1,17)
...
!       .600633794794645d0, .025574160612022d0, .134466754530780d0,
       .600633794794645d0/
...
!
!======================================================================
!  Rescale 1-D Gauss points and weights on interval [0,1]             |
!======================================================================
!
      do i=1,10
        do j=1,i
...
!
!======================================================================
!  Values of affine coordinates and weights for a Gaussian quadrature |
!  on a tetrahedron corresponding to Duffy's transformation           |
!======================================================================
!
!  ...loop through quadrature rules
      do np=1,7
        if (kwrite.eq.1) then
          write(*,7001) np
...
!  .....loop through integration points
 ...
 7003     format('const_T: ws = ',e12.5)
      ...
      enddo
!
!  ...CHECK : verify the integration rule
!
!  ...loop through orders p
      do np=1,7
!
...
!
!  .......evaluate the integral using 3D integration
          ...
          enddo
!
!  .......evaluate the integral using the 2D integration
          nord=(i+j+k+1)/2+1
          kint = NSELECT(nord)
          nint = NRGAUPO(kint)
          val2 = 0.d0
