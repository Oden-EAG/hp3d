...
!       compute unit tangent              
        ...
        call norm(Xi-void, s)
        if (s > GEOM_TOL) then
          write(*,*)'trace_tang : INCONSISTENCY! iedge = ',iedge
        endif
        dxdt(1:3,1:2)=0.d0
        do j=1,3
        ...
        write(*,*)'iedge,rt = ',iedge,dxdt(1:3,1)
...
!      
!       compute unit normal              
        call face_param(etype,iface,t, void,dxidt)
        call norm(Xi-void, s)
        if (s > GEOM_TOL) then
          write(*,*)'trace_tang : INCONSISTENCY! iface = ',iface
        endif
        dxdt(1:3,1:2)=0.d0
        do i=1,2
          do j=1,3
...
!
!     1st edge      
      if (abs(Xi(2)) < GEOM_TOLL .AND. &
          abs(Xi(3)) < GEOM_TOLL       ) then
        Iedge=1
        T(1)=Xi(1)
        return
      endif
!
!     2nd edge
      if (abs(Xi(1)+Xi(2)-1.d0) < GEOM_TOLL .AND. &
          abs(Xi(3))            < GEOM_TOLL       ) then
        Iedge=2
        T(1)=1.d0-Xi(1)
        return
...
! 
!     5th edge
      if (abs(Xi(1)+Xi(3)-1.d0) < GEOM_TOLL .AND. &
          abs(Xi(2)           ) < GEOM_TOLL       ) then
        Iedge=5
        T(1)=Xi(3)
        return
...
!      
!     6th edge
      if (abs(Xi(2)+Xi(3)-1.d0) < GEOM_TOLL .AND. &
          abs(Xi(1)           ) < GEOM_TOLL       ) then
        Iedge=6
        T(1)=Xi(3)
        return
      endif
...
!
!     3rd face
      if (abs(Xi(1)+Xi(2)+Xi(3)-1.d0) < GEOM_TOLL) then
        Iface=3 
        T(1)=Xi(2)
        T(2)=Xi(3)
        return
