...
!-------------------------------------------------------------------------------------------
!
subroutine paraview_attr_Order(Id, Idx)
!
...
!..file name and nickname for attribute
   fname = PHYSA(iattr)//"_vec_comp_"//comp//"_load_"//load//"_poly_"
   nick  = PHYSA(iattr)//"_vec_comp_"//comp//"_load_"//load//"_poly"
!
!..write to .h5 file
   call PVIS_VTK("Vector",  &
      trim(PARAVIEW_DIR)//trim(PREFIX)//"poly_order"//trim(fname)//postfix//".h5", &
      trim(nick),Idx, ic)
!
...
!
   90 continue
!
end subroutine paraview_attr_Order
