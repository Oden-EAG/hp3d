...
!----------------------------------------------------------------------
!
   subroutine logicC(Igrid, Mdle,Idec,                         &
...
!..get nodes of the modified element
   call logic_nodesC(Igrid,Mdle,nodesl, Nodm,Nrnodm)
!
!
...
!  ...printing
   if (iprint.eq.1) then
      ...
      do im=1,Nrnodm
         nod=Nodm(im)
         write(*,7002) im,nod,NODES(nod)%type,NODES_MG(nod)%orderC(Igrid),    &
...
!     ...printing
         if (iprint.eq.1) then
            write(*,7030) i,nod,NODES(nod)%type,NODES_MG(nod)%orderC(Igrid)
         endif
!
!======================================================================
!       NODE IS CONSTRAINED                                           |
...
!        ...node constrained by an edge
            case(11,12,13,37,38,39,47,48,49)
               write(*,7031) i,nod,NODES(nod)%type,NODES_MG(nod)%orderC(Igrid),icase,   &
                             NEDGC(nc),NEDG_CONS(1:2,nc)
!
!           ...node constrained by a face
            case default
               write(*,7032) i,nod,NODES(nod)%type,NODES_MG(nod)%orderC(Igrid),icase,   &
...
!        ...number of parent node dof's
            ndofHpp = NODES_MG(nodp)%orderC(Igrid)-1
!
!        ...number of parent dof's
...
!        ...number of parent node horizontal and vertical dof's
            call decode(NODES_MG(nodp)%orderC(Igrid), nordh,nordv)
...
!        ...parent mid-edge nodes (south,north)
            do ip=1,3,2
               ...
               ndofHp(ip) = NODES_MG(nodp)%orderC(Igrid)-1
               ndofEp(ip) = NODES_MG(nodp)%orderC(Igrid)
            enddo
!
...
!        ...parent mid-edge nodes (south,north)
            do ip=1,3,2
               nodp = iabs(NFACE_CONS(ip,nc))
               select case(NODES_MG(nodp)%master(Igrid))
               case(1)
                  ncflag(ip)=0
                  call locate(nodp,Nodm,Nrnodm, locp(ip))
                  ndofHp(ip) = 1
