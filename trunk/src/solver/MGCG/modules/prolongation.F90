...
!
   implicit none
!
!..order of approximation for edge or face parent nodes
   integer :: NORDERP(5)
!$OMP THREADPRIVATE(NORDERP)
!
!..actual and max number of nodes in the temporary data structure
   integer :: NR_NODES_PR, MAX_NODES_PR
!$OMP THREADPRIVATE(NR_NODES_PR, MAX_NODES_PR)
!
...
!  ...H1 prolongation coefficients
      real*8, allocatable :: coeffH(:,:)
!
!  ...H(curl) prolongation coefficients
      real*8, allocatable :: coeffE(:,:)
!
!  ...H(div) prolongation coefficients
      real*8, allocatable :: coeffV(:,:)
!
   endtype nodePR
!
!..nodes
...
!
!..nodal connectivities for an edge or face, order
   integer :: nodesl(9),nordp(5)
!
...
!..determine parent nodes and case numbers for the nodal sons
   ...
   case('medg')
      ...
      icasel(1:3)= (/11,12,13/)
      nordp(1) = nord; nordp(2:5)=-1
   case('mdlq')
      call face_nodesPR(Nodl, nodesl); nrp=9
      do i=1,5
         nordp(i) = NODES_PR(nodesl(4+i))%order
      enddo
      select case(kref)
      case(11)
...
!     wrt to the parent nodes
      call logicPR(icasel(is),nordp,naH,naE,naV, &
                   nrconH,nacH,constrH, &
                   nrconE,nacE,constrE, &
                   nrconV,nacV,constrV)
      if (iprint.eq.1) then
...
!  ...H1 dof
      if (ndofH.gt.0) then
         if (allocated(NODES_PR(nsonl)%coeffH)) deallocate(NODES_PR(nsonl)%coeffH)
         allocate(NODES_PR(nsonl)%coeffH(MDOFH,ndofH))
         NODES_PR(nsonl)%coeffH = 0.d0
         do kH=1,ndofH
            do kp=1,nrconH(kH)
               np = nacH(kp,kH)
               NODES_PR(nsonl)%coeffH(1:MDOFH,kH) = NODES_PR(nsonl)%coeffH(1:MDOFH,kH) &
...
!  ...H(curl) dof
      if (ndofE.gt.0) then
         ...
         do kE=1,ndofE
            do kp=1,nrconE(kE)
               np = nacE(kp,kE)
               NODES_PR(nsonl)%coeffE(1:MDOFE,kE) = NODES_PR(nsonl)%coeffE(1:MDOFE,kE) &
...
!  ...H(div) dof
      if (ndofV.gt.0) then
         if (allocated(NODES_PR(nsonl)%coeffV)) deallocate(NODES_PR(nsonl)%coeffV)
         allocate(NODES_PR(nsonl)%coeffV(MDOFV,ndofV))
         NODES_PR(nsonl)%coeffV = 0.d0
         do kV=1,ndofV
            do kp=1,nrconV(kV)
               np = nacV(kp,kV)
               NODES_PR(nsonl)%coeffV(1:MDOFV,kV) = NODES_PR(nsonl)%coeffV(1:MDOFV,kV) &
...
!          initiate father, son and generation number
   ...
   do while(nfath.gt.0)
      ...
      call nr_mdle_sons(NODES_PR(nfath)%type,NODES_PR(nfath)%ref_kind, nrsons)
...
7027        format('elem_nodes: stype,j,nson,ftype,jp,is = ', a5,2i3,2x,a5,2i3)
            call pause
         endif
!
...
!-----------------------------------------------------------------------
!
   subroutine logicPR(Icase,Nordp,NaH,NaE,NaV, &
                      NrconH,NacH,ConstrH, &
                      NrconE,NacE,ConstrE, &
                      NrconV,NacV,ConstrV)
!
   integer :: Icase,Nordp(5),NaH(9),NaE(9),NaV(9)
   integer :: NrconH,NrconE,NrconV,NacH,NacE,NacV
   double precision :: ConstrH,ConstrE,ConstrV
   dimension NrconH(MAXmdlqH),NrconE(MAXmdlqE),NrconV(MAXmdlqV), &
             NacH(MaxquadH,MAXmdlqH),ConstrH(MaxquadH,MAXmdlqH), &
...
!       parent mid-edge node
     nord = Nordp(1); loc=3; ndofH = nord-1; ndofE = nord
     select case(Icase)
     case(11); is=1
     case(12); is=2
     end select
...
!
!----------------------------------------------------------------------
   subroutine allocate_PR(Type,Nordp)
!
   character(len=4) :: Type
   integer :: Nordp(5)
!
!  ...locals
   integer :: ie,nord,nordh,nordv,ndofH,ndofE,ndofV,nodl
...
!
   ...
   select case(Type)
   case('medg')
     NORDERP(1) = Nordp(1); NORDERP(2:5)=-1
     NR_NODES_PR = 0
   ...
   case('mdlq')
     NORDERP(1:5) = Nordp(1:5)
     NR_NODES_PR = 0
     ! MAX_NODES_PR = (2**MAXGEN_PR+1)**2
     MDOFH=4; MDOFE=0
     do ie=1,4
       MDOFH = MDOFH + NORDERP(ie)-1
       MDOFE = MDOFE + NORDERP(ie)
     enddo
     nord = NORDERP(5)
     call decode(nord, nordh,nordv)
     MDOFH = MDOFH + (nordh-1)*(nordv-1)
     MDOFE = MDOFE + nordh*(nordv-1) + (nordh-1)*nordv
     MDOFV = nordh*nordv
   case default
...
!
!-----------------------------------------------------------------------
   subroutine build_edge_tree(Nodes_edge,Nordp)
!
   integer :: Nodes_edge(3),Nordp
!
!  ...list of nodes to be broken
   integer :: nlist
   integer, allocatable :: list(:), listb(:)
!
!  ...locals
...
7001   format('build_edge_tree:  Nodes_edge = ',3i6)
   ...
   nod = Nodes_edge(3)
   nord = Nordp; norder(1) = nord; norder(2:5)=-1
   if (allocated(NODES_PR)) call deallocate_PR
   call allocate_PR(NODES(nod)%type,norder)
!
...
!           nson = NODES(nod)%sons(js)
         ...
         if (ic.eq.nlist) then
            nlist = 2*ic
            allocate(listb(nlist)); listb(1:ic) = list(1:ic)
            call move_alloc(listb,list)
...
!
!-----------------------------------------------------------------------
   subroutine build_face_tree(Type,Nordp,Nfedg_or,Nodes_face)
!
   character(len=4) :: Type
   integer :: Nordp(5),Nfedg_or(4),Nodes_face(9)
!
!  ...list of nodes to be broken
   integer :: nlist
   integer, allocatable :: list(:), listb(:)
...
!
   ...
   ! if (Nodes_face(nrfn) .eq. 132) iprint = 1
   if (iprint.eq.1) then
     write(*,7002) Type,Nordp(1:nre+1),Nodes_face(1:nrfn)
7002   format('build_face_tree: Type,Nordf,Face nodes list = ',&
             a4,5i3,2x,9i3)
   endif
!
   if (allocated(NODES_PR)) call deallocate_PR
   call allocate_PR(Type,Nordp)
!
...
!  ...middle node
   nodl = NR_NODES_PR+1; NR_NODES_PR = nodl
   NODES_PR(nodl)%type = Type
   NODES_PR(nodl)%order = Nordp(nre+1)
   nod = Nodes_face(nrfn)
   NODES_PR(nodl)%nod = nod
   NODES_PR(nodl)%ref_kind = 0
   NODES_PR(nodl)%father = -1
...
!
   ...
   character(len=4) :: Type
   integer :: Maxgen
   integer :: max_nrvert, max_nredge, max_nrface, i
   integer :: e_0, e_1, e_2, v_0, v_1

   select case(Type)
      case('medg')
         max_nrnodes_pr = 3*2**Maxgen
      case('mdlq', 'mdlt')
