      ...
      do kp=1,GRID(Igrid)%constr(Iel)%nrconH_macro(kH)
         ...
         do ivar=1,NRHVAR*NRCOMS
            ZdofH_c(ivar,lH) = ZdofH_c(ivar,lH)   &
                             + GRID(Igrid)%constr(Iel)%constrH_macro(kp,kH)*zdofH_m(ivar,kH)
         enddo
      enddo
      ...
      do kp=1,GRID(Igrid)%constr(Iel)%nrconE_macro(kE)
         ...
         do ivar=1,NREVAR*NRCOMS
            zdofE_c(ivar,lE) = zdofE_c(ivar,lE)    &
                             + GRID(Igrid)%constr(Iel)%ConstrE_macro(kp,kE)*ZdofE_m(ivar,kE)
         enddo
      enddo
      ...
      do kp=1,GRID(Igrid)%constr(Iel)%nrconV_macro(kV)
         ...
         do ivar=1,NRVVAR*NRCOMS
            zdofV_c(ivar,lV) = zdofV_c(ivar,lV) &
                             + GRID(Igrid)%constr(Iel)%constrV_macro(kp,kV)*ZdofV_m(ivar,kV)
         enddo
      enddo
!...end of loop through macro-element H(curl) dof
   enddo
