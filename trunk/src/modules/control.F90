...
module control
  ...
  integer            :: INT_NEUMANN
  !
  !  ...extension flag for supporting construction of the extension
  !     operator (LEGACY)
  integer            :: NFLAG2GRID
  !
  !  ...input mode flag
...
contains
  ...
  subroutine read_control_from_file(fp)
    ...
    ncontrol=132
    open(unit=ncontrol,file=fp,form='formatted',access='sequential',status='unknown')
    !
...
8000 format(' GEOMETRY TOLERANCE = ',e12.5)
    !
...
!
  endsubroutine read_control_from_file
  !
end module control
