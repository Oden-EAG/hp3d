!-----------------------------------------------------------------------
!> @brief Routine performs fine mesh element to a coarse mesh element projections and computes
!         optimal p-refinement
!> @param[in]  Mdle                 - middle node number of the coarse element
!> @param[in]  Nr_mdle_sons         - number of child elements after isotropic refinement
!> @param[in]  Mdle_sons            - middle node numbers of the child elements
...
!> @param[out] rate_p               - rate of error reduction for p-only refinement
!> @date May 2024
!-----------------------------------------------------------------------
...
!  extraction vector to build the current one.
      call extraction_vector(0,nord_org,Nord_glob,nrdofmQ,nextract_prev,nextract)
...
!
            ...
            nextract = ZERO
!
            call extraction_vector(nord_prev,nord_mod,Nord_glob,nrdofmQ,nextract_prev,nextract)
!
            do iattr = 1,NRQVAR
               do l = 1,nrdofmQ
                  bwork(l,iattr) = Zbload(nextract(l),iattr)/Ap(nextract(l))
