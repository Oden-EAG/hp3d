...
!
 ...
 6001    format('celem_system: nrdoflH,nrdoflE,nrdoflV,nrdoflQ = ',4i4)
         write(*,6002) nrdofmH,nrdofmHE,nrdofmHEV,Nrdofm
 6002    format('celem_system: nrdofmH,nrdofmHE,nrdofmHEV,Nrdofm = ',4i4)
      endif
#endif
!
!***********************************************************************
...
!  ...end loop through L2 physics variables
      enddo
!..end loop through L2 dof for middle node
   enddo
!
!..compute the actual number of dof
...
!..determine the element local load vector and stiffness matrix
   ...
   do i=1,NR_PHYSA
     if((.not. PHYSAm(i)) .and. ((itest(i).eq.1) .or. (jtrial(i).eq.1))) then
       write(*,*) 'celem_system: INCONSISTENCY of PHYSAm and itest/jtrial. stop.'
       stop
     endif
   enddo
#endif
...
!  .....loop through physical attributes
 ...
 8002     format(' LOCAL STIFFNESS MATRIX  FOR ATTRIBUTES = ',2i2)
          ...
          enddo
        enddo
        write(*,*) 'celem_system: CONTINUE ? 1-Yes / 0-No'
        read(*,*) ians
