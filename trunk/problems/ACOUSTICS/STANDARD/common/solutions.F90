...
!..parameters
   real*8 :: rl, w0, p0, z_R,rk, theta_x, theta_y,rad_x,rad_y
...
!..sin solution vanishing on the boundary
   case(1)   
!
      p =dsin(x1*PI)*dsin(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
!   
!  ...1st order derivatives
      Gradp(1) = PI*dcos(x1*PI)*dsin(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
      Gradp(2) = PI*dcos(x2*PI)*dsin(x1*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
      Gradp(3) = PI*dcos(x3*PI)*dsin(x1*PI)*dsin(x2*PI)*(1.0D0,1.0D0)
! 
!  ...2nd derivative (3,3) matrix - Hessian
      Grad2p(1,1) = -PI**2*dsin(x1*PI)*dsin(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
      Grad2p(1,2) =  PI**2*dcos(x1*PI)*dcos(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
      Grad2p(1,3) =  PI**2*dcos(x1*PI)*dcos(x3*PI)*dsin(x2*PI)*(1.0D0,1.0D0)
      Grad2p(2,1) =  PI**2*dcos(x1*PI)*dcos(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
      Grad2p(2,2) = -PI**2*dsin(x1*PI)*dsin(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
      Grad2p(2,3) =  PI**2*dcos(x2*PI)*dcos(x3*PI)*dsin(x1*PI)*(1.0D0,1.0D0)
      Grad2p(3,1) =  PI**2*dcos(x1*PI)*dcos(x3*PI)*dsin(x2*PI)*(1.0D0,1.0D0)
      Grad2p(3,2) =  PI**2*dcos(x2*PI)*dcos(x3*PI)*dsin(x1*PI)*(1.0D0,1.0D0)
      Grad2p(3,3) = -PI**2*dsin(x1*PI)*dsin(x2*PI)*dsin(x3*PI)*(1.0D0,1.0D0)
!
...
!                   0.d0, sinx,  cosx /), shape(Rx), order=(/2,1/) )
   ...
   ! Ry=reshape( (/ cosy , 0.0d0, siny , &
   !                0.0d0, 1.0d0, 0.0d0, &
   !                -siny, 0.0d0, cosy  /), shape(Ry), order=(/2,1/) )
