...
!---------------------------------------------------------------------
!
   subroutine elem_residual_old(Mdle, Resid,Nref_flag)
...
!..factorize the test stiffness matrix
   ...
   call ZPPTRF(uplo, nrdofHH, AP, info) 
   if (info.ne.0) then
      write(*,*) 'elem_residual: AP info = ',info
      stop 1
   endif
!
   call ZTPTRS('U','C','N',nrdofHH,1,AP,BLOADH,MAXbrickHH,info1)
   if (info.ne.0) then
      write(*,*) 'elem: AP ZTPTRS: Mdle,info = ',Mdle,info
      stop 2
   endif
! 
   Resid = zdotc(nrdofHH,BLOADH,1,BLOADH,1)
!
   Nref_flag = 111

!
   end subroutine elem_residual_old
