...
!
   ...
   use uhm2
   use m_assembly, ONLY: NRDOF_TOT, NRDOF_CON, IPRINT_TIME
!
   implicit none
!   
...
!..print fancy header
   write(*,*)'                      '
   write(*,*)'// --  PRIMAL DPG FOR ACOUSTICS  -- //'
   write(*,*)'                      '
!
...
!        ...Multi-step adaptive h refinement
         case(24)
            ...
            do while ((nreflag.ne.1).and.(nreflag.ne.2).and.(nreflag.ne.3))
               ...
               write(*,*) 'REFINEMENT FLAG:h-refine=1,p-refine=2,hp-refine=3'
               write(*,*) '0.d0<FACTOR<1.d0'
               write(*,*) 'SOLVER:frontal=1,MUMPS=2,PARDISO=3,OMP MUMPS=4, PARDISO_SINGLE = 5'
...
!    ...Compute errors of different variables
      case(60)
         call exact_error
!
!     ...Compute residual and display history of residuals
      case(70)
