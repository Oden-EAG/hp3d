...
!----------------------------------------------------------------------
!    
   subroutine elemMG(Mdle, Itest,Itrial)
...
!  ...set the enriched order of approximation
      select case(etype)
         case('mdlb')        ; nordP = NODES_MG(Mdle)%orderC + nord_add_local*111
         case('mdln','mdld') ; nordP = NODES_MG(Mdle)%orderC + nord_add_local*1
         case('mdlp')        ; nordP = NODES_MG(Mdle)%orderC + nord_add_local*11
      end select
!
...
!      
      ...
      call elem_DPG_UWEAK_ACOUSTICS_MG(mdle,nord_add_local,nrTest,nrTrial,                     &
...
!----------------------------------------------------------------------
!    
   subroutine elem_DPG_UWEAK_ACOUSTICS_MG(Mdle,nord_add_local,nrTest,nrTrial,     &
                                       nrdofHH,nrdofVV,nrdofH,nrdofV,nrdofQ)
!
   use data_structure3D
   use mg_data_structure
...
!
   if (info.ne.0) then
      write(*,*) 'elemMG: Gram ZPOTRF: Mdle,info = ',Mdle,info
      write(*,*) 'elemMG: nrTest, nrTrial = ',nrTest, nrTrial
      call graphb
      stop 1
   endif
!
...
!
!   
   end subroutine elem_DPG_UWEAK_ACOUSTICS_MG