...
!----------------------------------------------------------------------
!
   subroutine celem_sysMG(Mdle,Idec,                                &
...
!  ...number of dofs for a SINGLE H1,H(curl),H(div),L2 component
      ...
      if (iprint.eq.1) then
        write(*,5001) Mdle
 5001   format(' celem_sysMG: DEBUGGING FOR Mdle = ',i8)
...
!  ...loop through higher order nodes, middle node first
      ...
      call get_index(nod, index)
      kold = k
      call find_ndofC(nod, nvoid,nvoid,nvoid,ndofQ)
      l = nrdofmHEV
!
