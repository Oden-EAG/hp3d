...
! ----------------------------------------------------------------------
!
   subroutine patch_assembly
!
   use data_structure3D, only: NRNODS
   use patch_info     
   use mg_data_structure
   use macro_grid_info,  only: MACRO_ELEM, DLOC
...
!
   ...
   integer, allocatable, save :: npH(:), npE(:), npV(:), nvisit(:)
!$omp threadprivate(npH,npE,npV,nvisit)   
   integer, allocatable, save :: nodl_aux(:), nod_macro(:)
!$omp threadprivate(nodl_aux, nod_macro)   
   integer, allocatable, save :: ndofH_macro(:), ndofE_macro(:), ndofV_macro(:)
...
!        ...avoid repetition
            ...
            call locate(nod,PTCH(ip)%nodl,nrnodp,nloc)
            if (nloc .ne. 0) then
               inod = inod + 1
               nodl_aux(inod) = nod
               nvisit(nod) = 1
...
!  ...the node ordering has to agree with local dof 
      PTCH(ip)%nodl(1:nrnodp) = nodl_aux(1:nrnodp)
!
      deallocate(nodl_aux)
!
...
!        ...check if the nod is in the patch            
         ...
         PTCH(ip)%mdlel(jel)%ndof = lp
!     ...store connectivity map for this mdle in the patch 
         allocate(PTCH(ip)%mdlel(jel)%lcon(l)); PTCH(ip)%mdlel(jel)%lcon = lcon
         deallocate(lcon)
         deallocate(nod_macro)
         deallocate(ndofH_macro, ndofE_macro, ndofV_macro)
...
!  ...total number of dof in the patch      
      nrdof = PTCH(ip)%nrdof
! 
      allocate(PTCH(ip)%zAp(nrdof*(nrdof+1)/2)) 
      PTCH(ip)%zAp = ZERO
!      
!  ...number of mdle nodes contributing to the patch
      nrmdle = PTCH(ip)%nrmdle
...
!     ...allocate connectivity array
         allocate(lcon(ndof_macro))
!
         lcon = PTCH(ip)%mdlel(jel)%lcon; deallocate(PTCH(ip)%mdlel(jel)%lcon)
!         
...
#if C_MODE
                        if (k2 .ge. k1 ) then
                           PTCH(ip)%zAp(ij) = PTCH(ip)%zAp(ij) &
                                            + DLOC(iel)%zstiff(kk)
                        else
                           PTCH(ip)%zAp(ij) = PTCH(ip)%zAp(ij) &
                                            + conjg(DLOC(iel)%zstiff(kk))
                        endif
#else                                                  
                        PTCH(ip)%zAp(ij) = PTCH(ip)%zAp(ij) &
                                         + DLOC(iel)%zstiff(kk)
...
!
#if C_MODE      
      call ZPPTRF('U', nrdof, PTCH(ip)%zAp, info)
#else
      call DPPTRF('U', nrdof, PTCH(ip)%zAp, info)
#endif      
      if (info .ne. 0) then 
...
!----------------------------------------------------------------------
!
   subroutine patch_connect_maps

   use data_structure3D,   only: NRNODS
   use mg_data_structure,  only: NODES_MG
   use macro_grid_info,    only: NRELES_COARSE, MACRO_ELEM
   use patch_info
!
   IMPLICIT NONE
!
...
!  ...H(curl) dof 
      do i =1,nrnodp
         ...
         do j=1,NODES_MG(nod)%nod_ndof(2)
            l=l+1
            PTCH(k)%lcon(l) = nrdof_H + npE(nod)+j
         enddo
      enddo
!        
!  ...H(div) dof 
      do i = 1,nrnodp
         ...
         do j=1,NODES_MG(nod)%nod_ndof(3)
            l=l+1
            PTCH(k)%lcon(l) = nrdof_H + nrdof_E + npV(nod)+j
         enddo
      enddo
