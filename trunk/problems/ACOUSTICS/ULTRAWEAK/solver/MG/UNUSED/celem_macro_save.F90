...
!
   ...
   use assembly,         ONLY: NR_RHS, MAXDOFS, NEXTRACT, IDBC, ZDOFD,   &
...
!..global matrices
   ...
   ! complex*16, allocatable  :: Abi_vect(:)
...
!        ...Assemble global load vector
            ! ii = nrdofb*nrdofi+i
            ! Abi_vect(ii) = Abi_vect(ii) + zload(k1)
...
!                    ...sparse storage 
                  ...
                  ! k = (k1-1)*ndof + k2
                  ! Abi_vect(i+(j-1)*nrdofb) = Abi_vect(i+(j-1)*nrdofb) + ztemp(k)
                  ! Abi_ext(i,j) = Abi_ext(i,j) + ztemp(k)
                  Abi(i,j) = Abi(i,j) + ztemp(k)
               endif
            enddo
         else 
!        ...Assemble global load vector
...
!..call pardiso to solve
   ! call pardiso_solve_vect(IAbb(1:nrdofb+1),JAbb,SAbb,'H', &
                           ! nnz_tot,Abi_vect,nrdofb,nrdofi+1,Abi,bb)
!
   call pardiso_solve(IAbb(1:nrdofb+1),JAbb,SAbb,'H', &
                           nnz_tot,nrdofb,nrdofi,Abi)
