...
! ----------------------------------------------------------------------
!
   subroutine macro_elem_info(MdleC, Nrdof,Nrdofb, Nrnod_macro, Nod_macro,  &
...
!-----------------------------------------------------------------------
!
   integer, intent(in)   :: mdleC, Nrnod_macro
...
!  ...fill in the lists for macro element nodes      
      do i =1, nrnodm-1 ! no need to check the mdle node
         nod = nodm(i)
...
!        ...update the H(div) dof counter
      ...
      enddo
!
!  ...compute offsets for L2 dof (all of them are bubbles)
      i = nrnodm
      nod = nodm(i)
!  ...update the L2 dof counter
      nrdof_Qb = nrdof_Qb + ndofmQ(i)
