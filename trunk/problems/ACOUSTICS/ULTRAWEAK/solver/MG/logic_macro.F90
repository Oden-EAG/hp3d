...
!-----------------------------------------------------------------------
!
   subroutine logic_macro(MdleC, Nod_macro,Nrnod_macro,               &
...
!
!..globals in
   integer, intent(in)  :: MdleC, Nod_macro(Nrnod_macro), Nrnod_macro
...
! 
 ...
 7001 format('logic_macro: COARSE LOCAL ELEMENT NODES FOR MdleC = ',i6)
      do i = 1,nrv+nre+nrf
         ...
         call ndof_nod(NODES(nod)%type,NODES_MG(nod)%orderC, &
                       ndofH,ndofE,ndofV,nvoid)
         write(*,7000) i,nod,NODES(nod)%type,NODES_MG(nod)%orderC, &
                       naHl(i),ndofH, &
                       naEl(i),ndofE, &
                       naVl(i),ndofV
      enddo 
...
!  ...if the node belongs also to the macro-element then connect it to itself
      if (loc.ne.0) then
         ...
         call inject_dof(NODES(nod)%type, NODES_MG(nod)%orderC,NODES(nod)%order,   &
                         InjH, InjE, InjV)
         do j=1,NdofmH(i)
...
!  ...establish the injections between edge and element dof
      ...
      enddo
!
      call build_edge_tree(nodes_edge,NODES_MG(nod)%orderC)
!
...
!  ...determine the face parent nodes order for the prolongation operator
      do i=1,nrvf+1
        nordp(i) = NODES_MG(nodesl_face(nrvf+i))%orderC
      enddo
!
