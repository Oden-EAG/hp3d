...
! ----------------------------------------------------------------------
   ...
   subroutine mumps_interface(mtype)
!      
...
!..memory allocation for mumps
   ...
   allocate(MUMPS_PAR%RHS(MUMPS_PAR%N)) ; MUMPS_PAR%RHS = ZERO
!
...
!$OMP CRITICAL
         MUMPS_PAR%RHS(i) = MUMPS_PAR%RHS(i) + ZLOADc(k1)
!$OMP END CRITICAL
...
! 
!
   end subroutine mumps_interface
