...
!  ...wavenumber
      rk = OMEGA/1.5d0
!
      xshift = x1 - 6.75d0; yshift = x2 - 6.75d0; zshift = x3 - 4.2d0
      theta_x = 0.d0; theta_y=180.d0;
      rad_x = theta_x*pi/180.d0;  rad_y = theta_y*pi/180.d0;
!
      sinx=sin(rad_x); cosx = cos(rad_x);
      siny=sin(rad_y); cosy = cos(rad_y);
...
!  ...pressure
      p0 = 1.0d0*(2.0d0/pi/w0**2)**0.25;
      p  = p0*exp(-ZIMG*OMEGA/1.5d0*z)*(1-0.5d0*(r**2/w0**2))*exp(-r**2/w0**2)
!
!  ...derivatives with respect to x,y,z
      bump = 1.d0/(1.d0 - 0.5d0*r**2/w0**2)
      p_x = -p * x/w0**2 * (1.d0 + bump)
      p_y = -p * y/w0**2 * (1.d0 + bump)
      p_z = (-ZIMG*OMEGA)* p
!
!  ...second derivatives with respect to x,y,z
      p_xx = (-1.d0/w0**2)* (p+x*p_x) - p * x**2/w0**2 * bump**2
      p_xy = (-1.d0/w0**2)* x*p_y*(1.d0 + bump) - p_x * y/w0**2 * bump**2
      p_xz = (-1.d0/w0**2)* x*p_z
      p_yx = p_xy
      p_yy = (-1.d0/w0**2)*(p+y*p_y) - p * y**2/w0**2 * bump**2
      p_yz = (-1.d0/w0**2)* y*p_z
      p_zx = p_xz
      p_zy = p_yz
      p_zz = (-ZIMG*OMEGA)* p_z
