...
!
   ...
   integer              :: vert_singular(8)
   integer              :: edge_singular(12)
   integer              :: i,j,nod,nodp,iv,ie,nrv,nre
!  ...work space for elem_nodes
   integer              :: nodesl(27), norientl(27)
!..geometry dof
   real*8  :: xnod(3,MAXbrickH), hmin, lamda
...
!     ...look for a singular edge or its father
         do ie = 1, nre
            ...
            if (NODES(nodp)%type .ne. 'medg') cycle
            do j = 1,12
...
!
   lamda = OMEGA/(2.0d0*PI-1e-9)
   if (hmin.gt. 1.d0/lamda .or. found) then
      Iref=1
   else
      Iref=2
   endif
...
!
   ...
   son  = nod
!
   do while (NODES(son)%father .gt. 0)
      nodp = NODES(son)%father
      son  = nodp
   enddo   
!
!
