...
for j = 0:15
    ...
    for i = 1:NRELES
        ...
        if (n(1)*max(x)+n(2)*max(y)+n(3)*max(z) >= d)
            ...
            patch('Vertices', vert, 'Faces', faces, 'FaceColor', col, 'FaceAlpha',0.9, 'EdgeColor','k', 'EdgeAlpha',1)
            hold on;
        end
    end
    
    h(1) = plot(NaN,NaN,'s','MarkerFaceColor','b','MarkerSize',20,'LineWidth',2);
    h(2) = plot(NaN,NaN,'s','MarkerFaceColor','c','MarkerSize',20,'LineWidth',2);
...
%     set(gca,'position',[0.01 0.01 0.98 0.98],'units','normalized')
    file1=strcat('mesh',num2str(imesh),'.png');
