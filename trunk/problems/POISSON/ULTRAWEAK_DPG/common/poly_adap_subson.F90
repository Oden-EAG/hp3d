...
subroutine poly_adap_subson_new(kref_loc,Mdle,nrdofgQ,Nord_old,Nord_glob,subson_overlap,base_error, &
                                Ap,zbload,Awork_prev,Bwork_prev,Nextract_prev,Polyflag,&
...
real(8),    intent(in)  ::  base_error !error at order at which we start subson telescopic solves
real(8), dimension(nrdofgQ,nrdofgQ),    intent(in) :: Ap
real(8), dimension(nrdofgQ,MAXEQNQ),    intent(in) ::zbload
real(8), dimension(nrdofgQ,nrdofgQ),    intent(in)   :: Awork_prev
real(8), dimension(nrdofgQ,MAXEQNQ),    intent(in)   :: Bwork_prev
integer, intent(in) :: Nextract_prev(*)
integer, intent(in) :: subson_overlap(*)
integer, intent(out) :: Polyflag
...
Awork_subson(1:nrdofgQ,1:nrdofgQ) = Awork_prev(1:nrdofgQ,1:nrdofgQ)
Bwork_subson(1:nrdofgQ,1:MAXEQNQ) = Bwork_prev(1:nrdofgQ,1:MAXEQNQ)
...
if(etype .eq. 'mdlb') then
    ...
    do iattr = 1,NRQVAR
        coeff_sum = ZERO
        call coeff_polynomial_aniso(Nord_old,Nextract_prev_subson(1:nrdofmQ),etype,Bwork_subson(1:nrdofmQ,iattr),coeff_sum)
    ...
    do k = 1,3
        ...
        do iattr = 1,NRQVAR
            ...
            call fine_to_subson_projection_error_new(kref_loc,Bwork_subson(1:nrdofmQ,iattr),Nextract_subson,subson_overlap,&
                                                    nrdofmQ,nrdofgQ,iattr,Mdle,proj_error,&
                                                    weights_fine_store,quad_point_store,nint_pp_store,&
                                                    shap3DQ_fine_store,shap3DQ_coarse_store)
        ...
        error_rate_aniso(k) = (base_error - proj_error_net)/(abs(real(nrdofmQ*NRQVAR,8) - real(nrdof_old*NRQVAR,8)))
        poly_flag_store(k) = Nord_mod
        ...
        do iattr = 1,NRQVAR
            ...
            call coeff_polynomial_aniso(Nord_mod,Nextract_prev_subson(1:nrdofmQ),etype,Bwork_subson(1:nrdofmQ,iattr),coeff_sum)
