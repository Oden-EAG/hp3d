...
subroutine solution(Xp, p,Gradp,Grad2p)
!
   use data_structure3D
   use common_prob_data, only : PI, OMEGA, ISOL, NPX,NPY,NPZ
...
!..parameters
   real*8 :: rl, w0, p0, z_R,rk, pi_mod, rn(3)
...
!..functions and their derivatives
   ...
   real*8     :: ord_x, ord_y, ord_z
   real*8     :: p1, dp1, d2p1, p2, dp2, d2p2, p3, dp3, d2p3
! 
!---------------------------------------------------------------------------------------
!
...
!..polynomial in x dimension
   ...
   ord_x=real(NPX,8); ord_y=real(NPY,8); ord_z=real(NPZ,8)
      p1 = x1**ord_x;  p2 = x2**ord_y; p3 = x3**ord_z
!
      select case(NPX)
      case(0)
         dp1 = 0.d0; d2p1 = 0.d0
      case(1)
         dp1 = 1.d0; d2p1 = 0.d0
      case default
         dp1  = ord_x * x1**(ord_x-1.d0)
         d2p1 = ord_x * (ord_x-1.d0) * x1**(ord_x-2.d0)
      end select         
      select case(NPY)
      case(0)
         dp2 = 0.d0; d2p2 = 0.d0
      case(1)
         dp2 = 1.d0; d2p2 = 0.d0
      case default
         dp2  = ord_y * x2**(ord_y-1.d0)
         d2p2 = ord_y * (ord_y-1.d0) * x2**(ord_y-2.d0)
      end select  
      select case(NPZ)
      case(0)
         dp3 = 0.d0; d2p3 = 0.d0
      case(1)
         dp3 = 1.d0; d2p3 = 0.d0
      case default
         dp3  = ord_z * x3**(ord_z-1.d0)
         d2p3 = ord_z * (ord_z-1.d0) * x3**(ord_z-2.d0)
      end select  
...
!  ...second derivatives
      Grad2p(1,1) = d2p1 * p2   * p3
      Grad2p(1,2) = dp1  * dp2  * p3
      Grad2p(1,3) = dp1  * p2   * dp3
      Grad2p(2,1) = Grad2p(1,2)
      Grad2p(2,2) = p1  * d2p2 * p3
      Grad2p(2,3) = p1  * dp2 * dp3
      Grad2p(3,1) = Grad2p(1,3)
      Grad2p(3,2) = Grad2p(2,3)
      Grad2p(3,3) = p1  * p2  * d2p3
!
!..sin solution 
   case(1)   
!
...
!..plane wave
   case(3)
      x1 = x1 + 0.0d0; x2 = x2 + 0.0d0; x3 = x3 + 0.5d0
      rn = (/1.0d0,1.0d0,0.0d0/)

      zf_x = exp(-ZIMG*OMEGA*x1*rn(1))
      zf_y = exp(-ZIMG*OMEGA*x2*rn(2))
      zf_z = exp(-ZIMG*OMEGA*x3*rn(3))
!      
!  ..first order derivatives      
      dzf_x = -ZIMG*OMEGA*zf_x*rn(1)
      dzf_y = -ZIMG*OMEGA*zf_y*rn(2)
      dzf_z = -ZIMG*OMEGA*zf_z*rn(3)
!
! ...second order derivatives      
      ddzf_x = -OMEGA**2*zf_x*rn(1)
      ddzf_y = -OMEGA**2*zf_y*rn(2)
      ddzf_z = -OMEGA**2*zf_z*rn(3)
!
      p = zf_x*zf_y*zf_z
...
!  ...wavenumber
      rk = OMEGA
!  ...shift the origin   
!  ...diagonal starting from the corner
      xshift = x1 + 0.02d0; yshift = x2 + 0.02d0; zshift = x3 + 0.02d0
      theta_x = 35.d0; theta_y=-45.d0; theta_z = 0.d0

!  ...horizontal starting from the middle   
      ! xshift = x1 + 0.02d0; yshift = x2 + 0.02d0; zshift = x3 - 0.5d0
      ! theta_x = 90.d0; theta_y=0.d0; theta_z = 45.d0

!  ...horizontal starting from the corner   
      ! xshift = x1 + 0.02d0; yshift = x2 + 0.02d0; zshift = x3 + 0.02d0
      ! theta_x = 90.d0; theta_y=0.d0; theta_z = 45.d0

      rad_x = theta_x*pi/180.d0;  rad_y = theta_y*pi/180.d0; 
      rad_z = theta_z*pi/180.d0
      
      sinx=sin(rad_x); cosx = cos(rad_x);
      siny=sin(rad_y); cosy = cos(rad_y);
      sinz=sin(rad_z); cosz = cos(rad_z);
!
...
! 
      Rz(1,1:3) = (/cosz, -sinz,  0.0d0/)
      Rz(2,1:3) = (/sinz,  cosz,  0.0d0/)
...
!   
      ...
      call DGEMM('N','N',3,3,3,1.0d0,Rxy,3,Rz,3,0.0d0,Rxyz,3)
!   
!  ...new coordinates after rotation
      x = Rxyz(1,1)*xshift+Rxyz(1,2)*yshift+Rxyz(1,3)*zshift
      y = Rxyz(2,1)*xshift+Rxyz(2,2)*yshift+Rxyz(2,3)*zshift
      z = Rxyz(3,1)*xshift+Rxyz(3,2)*yshift+Rxyz(3,3)*zshift

!  ...change of variable derivatives
      dxdxs = Rxyz(1,1) ; dxdys = Rxyz(1,2) ; dxdzs = Rxyz(1,3)
      dydxs = Rxyz(2,1) ; dydys = Rxyz(2,2) ; dydzs = Rxyz(2,3)
      dzdxs = Rxyz(3,1) ; dzdys = Rxyz(3,2) ; dzdzs = Rxyz(3,3)
