!---------------------------------------------------------------------
!> @brief   Main driver for the UW elasticity problem
!!
!> @date    July 2023
!---------------------------------------------------------------------
   program main
...
!     ...print header
         write(6,*)
         write(6,*) '//                                //'
         write(6,*) '// -- MPI ELASTICITY ULTRAWEAK -- //'
         write(6,*) '//                                //'
         write(6,*)
      endif
      flush(6)
...
!  ...Set flags for de-/activating variables
      PHYSAm(1:4) = (/.true.,.true.,.true.,.true./)
!  ...Set flags for in-/homogeneous Dirichlet BCs
      PHYSAd(1:4) = (/.false.,.false.,.false.,.false./)
!
...
!
!  TODO: propagate flag for impedence BC
!
      if (JOB .ne. 0) then
         call exec_job
      else
...
!----------------------------------------------------------------------
!> @brief   Main routine for ROOT MPI rank
!!
!> @date    July 2023
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
!> @brief   Main routine for non-ROOT MPI ranks
!!
!> @date    July 2023
!----------------------------------------------------------------------
...
!
!  ...auxiliary variables
      integer :: idec, i, r, lb, count, src, ierr
!
!----------------------------------------------------------------------
!
      if (RANK .eq. ROOT) then
