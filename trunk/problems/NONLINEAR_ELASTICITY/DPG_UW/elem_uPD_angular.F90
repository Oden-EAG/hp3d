...
!
!     DISPLACEMENT GRADIENT   | LINEAR MOMENTUM BALANCE | PK1 CONSTITUTIVE LAW   | ANGULAR MOMENTUM BALANCE
!
!  |   \tau \in H(div)^3      |       v \in (H1)^3      | \chi \in (L^2)^3x3     |  \Xi \in L^2(\Omega;\Skew)
!
!  - <\delta \hat u,(\tau n)> +            0
!  +               0          -   < \delta \hat t ,v >                       
!  + ( \delta u , div(\tau) ) +            0           
!  +               0          + ( \delta P , grad(v) )  + (    \delta P , \chi)  + ( (\delta P) D^T - D (\delta P)^T , \Xi )
!  + ( \delta D , \tau )      +            0            - (L : \delta D , \chi)  + ( P (\delta D)^T - (\delta D) P^T , \Xi )
!  = 
!                             + (f , v)                 + ( K(I+D) - P ,\chi)    - ( P (I+D)^T - (I+D) P^T , \Xi )
!  - (u,div(\tau))            + <\hat t ,v>             
!  + < \hat u,(\tau n)>       - (P,grad(v))
!  - (D,\tau)
!------------------------------------------------------------------------------------------
!
subroutine elem_DPG_UW_uPD_angular(Mdle)
...
!
! ....hyperelasticity 
      real*8 :: W,dWdF(3,3),d2WdF(3,3,3,3), gradu(3,3) , PK1(3,3), u(3), uhat(3), trac(3), khh(3,3)
      integer:: imat
...
!  ...miscellaneous
      ...
      real*8  :: weight,wa,rjac,brjac,tmp,diffmax,dmax,dtmp(3,3),ptmp(3,3)
! 
...
!
              enddo; enddo
            enddo
!      ...end of outer loop over \delta D`
          enddo;enddo
        enddo
! 
!
!     P   A   R   T      4
!
!     L I N E A R I Z E D   A N G U L A R   M O M E N T U M   B A L A N C E
!
! + ( (\delta P) D^T - D (\delta P)^T , \Xi )
! + ( P (\delta D)^T - (\delta D) P^T , \Xi )
! =
! + ( (I+D) P^T - P (I+D)^T , \Xi )
! 
!     By FOSLS  
!     \Xi = (\delta P`)D^T - D(\delta P`)^T + P(\delta D`)^T - (\delta D`)P^T
!
!       get the skewsymm part of Kirchhoff stress P*F^T (x2)
        khh=matmul(PK1,transpose(DEL+gradu))
        khh=khh-transpose(khh)
!        
!  ...loop over optimal test function \delta P' 
        do k1=1,nrdofQ
          jcomp = 0
...
!
!     T E R M   3: 
!    -( P(I+D)^T - (I+D)P^T , (\delta P`)D^T - D(\delta P`)^T )
!                        
          ! get the skewsymm matrix (\delta P`)D^T - D(\delta P`)^T
            dtmp=0.d0
            dtmp(k,:)=Gradu(:,l)
            dtmp(:,k)=dtmp(:,k)-Gradu(:,l)
            tmp = (khh(1,2)*dtmp(1,2)+khh(1,3)*dtmp(1,3)+khh(2,3)*dtmp(2,3))
            FOSLS_Load(m1,1) = FOSLS_Load(m1,1)                   &
                             - 2.d0*tmp*shapQ(k1)*weight
! 
!     S T I F F N E S S   M A T R I X
!
!     T E R M   5: 
!   + ( (\delta P) D^T - D (\delta P)^T , (\delta P`)D^T - D(\delta P`)^T )
! 
            do k3=1,nrdofQ
              icomp = 0
              do j=1,3; do i=1,3
                ...
                ptmp=0.d0
                ptmp(i,:)=Gradu(:,j)
                ptmp(:,i)=ptmp(:,i)-Gradu(:,j)
                tmp = (ptmp(1,2)*dtmp(1,2)+ptmp(1,3)*dtmp(1,3)+ptmp(2,3)*dtmp(2,3))
                FOSLS_Stress(m3,m1) = FOSLS_Stress(m3,m1)                          &
                                    + 2.d0*tmp*shapQ(k1)*shapQ(k3)*weight
              enddo;enddo
            enddo
!
!     T E R M   6:  
!   + ( P (\delta D)^T - (\delta D) P^T , (\delta P`)D^T - D(\delta P`)^T )
!
            do k5=1,nrdofQ
              icomp = 0
              do j=1,3; do i=1,3
...
!
                FOSLS_Grad(m5,m1) = FOSLS_Grad(m5,m1)                              &
                                  + 2.d0*tmp*shapQ(k1)*shapQ(k5)*weight  
              enddo;enddo
            enddo
!    ...end of \delta P` loop
          enddo;enddo
...
!
!     T E R M   4:
!    -( P(I+D)^T - (I+D)P^T ,  P(\delta D`)^T - (\delta D`)P^T )
!
           ! get the skewsymm matrix (\delta P`)D^T - D(\delta P`)^T
            ptmp=0.d0
            ptmp(:,k)=PK1(:,l)
            ptmp(k,:)=ptmp(k,:)-PK1(:,l)
            tmp = (khh(1,2)*ptmp(1,2)+khh(1,3)*ptmp(1,3)+khh(2,3)*ptmp(2,3))
            FOSLS_Load(m1,1) = FOSLS_Load(m1,1)                     &
                             - 2.d0*tmp*shapQ(k1)*weight
! 
!     S T I F F N E S S   M A T R I X
!
!     T E R M   8: 
!   + ( P (\delta D)^T - (\delta D) P^T , P(\delta D`)^T - (\delta D`)P^T )
! 
            do k3=1,nrdofQ
              icomp = 0
              do j=1,3; do i=1,3
...
!     F O S L S   S T I F F N E S S   M A T R I X
!
!     T E R M   3 + 7 :   we take the transpose of term 2 + 6
!
...
! 
      deallocate(FullDPG)
!
end subroutine elem_DPG_UW_uPD_angular
! 