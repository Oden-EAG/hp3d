...
!
!     DEFORMATION GRADIENT    | LINEAR MOMENTUM BALANCE | PK1 CONSTITUTIVE LAW 
!
...
!------------------------------------------------------------------------------------------
!
subroutine elem_DPG_UW_general(Mdle)
...
!     (TRANSPOSED) stiffness and load matrices for the enriched test space 
      ...
      real*8, dimension(9*MAXbrickQ,3*MAXbrickVV+3*MAXbrickHH) :: EnrFieldDefGrad!,EnrFieldStressc
      real*8, dimension(3*MAXbrickVV+3*MAXbrickHH,MAXNRHS_MOD) :: EnrLoad
!     (TRANSPOSED) ENRICHED STIFFNESS matrix (excludes least squares part)
      real*8, dimension(3*(MAXbrickH-MAXmdlbH)+3*(MAXbrickV-MAXmdlbV)+21*MAXbrickQ,3*MAXbrickVV+3*MAXbrickHH) :: EnrStiffnessT
...
!     (TRANSPOSED) stiffness and load matrices for Least Squares equations
      ...
      real*8, dimension(9*MAXbrickQ,18*MAXbrickQ) :: FOSLS_DefGrad
...
!
! ....hyperelasticity 
      real*8 :: W,dWdF(3,3),d2WdF(3,3,3,3), Ftensor(3,3) , PK1(3,3), u(3), uhat(3), trac(3)
      integer:: imat
...
!  ...initialize the enriched local element stiffness matrices and load vectors
      ...
      EnrFieldDefGrad=ZERO;FOSLS_Stress=ZERO; FOSLS_DefGrad=ZERO
...
!  .....compute the approximate solution on the PHYSICAL element
        ...
        Ftensor = reshape(solQ_up(13:21),(/3,3/))
...
!    (I,\tau) - (u,div(\tau)) - (F,\tau)
            EnrLoad(m1,1:NR_RHS) = EnrLoad(m1,1:NR_RHS) + weight                  &
                                 * (  shapVV(jcomp,k1)                            &
                                    - u(jcomp)*divVV(k1)                          &
                                    - dot_product(Ftensor(jcomp,:),shapvv(:,k1)) )
! 
! 
!
!           G R A M   M A T R I X
...
!         
! 
!           T E R M   2 :    ( \delta F , \tau )
! 
! 
!        ...INNER loop through trial dofs for deformation gradient
            do k5 = 1, nrdofQ
!             icomp ranges 1 through 9, corresponds to number of copy
              icomp = 0
!             j is the column of \delta F, i is the row
              do j=1,3; do i=1,3
                icomp = icomp + 1
!               dof number
                m5 = (k5-1)*9 + icomp
!               contributions come from matching rows of \tau and \delta F (jcomp and i)
                if (jcomp.ne.i) cycle
                EnrFieldDefGrad(m5,m1) = EnrFieldDefGrad(m5,m1)             &
                                       + shapQ(k5)*shapVV(j,k1)*weight
              enddo; enddo
!        ...end of deformation gradient inner loop
            enddo
...
!    ( K(F) - P , \chi )
!     
!    by FOSLS, \chi = \delta P` - L : \delta F`
!
...
!
            do k5=1,nrdofQ
              ...
              do j=1,3; do i=1,3
                ...
                m5 = (k5 - 1)*9 + icomp
!
                FOSLS_DefGrad(m5,m1) = FOSLS_DefGrad(m5,m1)                     &
...
!
!
!  ...loop over optimal test function \delta F` 
        do k1=1,nrdofQ
          jcomp = 0
          do l=1,3; do k=1,3
            jcomp = jcomp + 1
...
!
            do k3=1,nrdofQ
              ...
              do j=1,3; do i=1,3
                ...
                m3 = (k3-1)*9 + icomp
!
                FOSLS_DefGrad(m3,m1) = FOSLS_DefGrad(m3,m1)                          &
                                     + shapQ(k1)*shapQ(k3)                           &
...
!
      FOSLS_Stress(1:9*nrdofQ,9*nrdofQ+1:18*nrdofQ) = transpose(FOSLS_DefGrad(1:9*nrdofQ,1:9*nrdofQ))
!
! 
! 
!
...
!
 ...
 6001     format('i = ',i4,'  ',19(/,10e12.5))
        ...
        write(*,*) 'EnrFieldOmega = '
        do i=1,3*nrdofVV+1
          write(*,6001) i,EnrFieldDefGrad(1:9*nrdofQ,i)
        enddo
...
!  ...EnrTraceDispl (H1)
      ...
      EnrStiffnessT(kmin+1:kmax,1:enrdof)=EnrTraceDispl(1:(kmax-kmin),1:enrdof)
!  ...EnrTraceStress (H(div))
      kmin=kmax
      kmax=kmin+ndofphysics(2)
      EnrStiffnessT(kmin+1:kmax,1:enrdof)=EnrTraceStress(1:(kmax-kmin),1:enrdof)
!  ...EnrFieldDispl (L2)
      kmin=kmax
      kmax=kmin+ndofphysics(3)
      EnrStiffnessT(kmin+1:kmax,1:enrdof)=EnrFieldDispl(1:(kmax-kmin),1:enrdof)
!  ...EnrFieldStress (L2)
      kmin=kmax
      kmax=kmin+ndofphysics(4)
      EnrStiffnessT(kmin+1:kmax,1:enrdof)=EnrFieldStress(1:(kmax-kmin),1:enrdof)
!  ...EnrFieldDefGrad (L2)
      kmin=kmax
      kmax=kmin+ndofphysics(5)
      EnrStiffnessT(kmin+1:kmax,1:enrdof)=EnrFieldDefGrad(1:(kmax-kmin),1:enrdof)
!  ...store in right position in EnrEverything
      EnrEverything(1:enrdof,1:kmax) = transpose(EnrStiffnessT(1:kmax,1:enrdof))
!  ...EnrLoad
      kmin=kmax
      kmax=kmin+NR_RHS
...
!  ...Build full DPG matrix (stiffness + load) in one go
      ...
      call DGEMM(transa,transb,m,n,k,1.d0,EnrStiffnessT,l,EnrEverything,l,0.d0,FullDPG,m)!3*(MAXbrickH-MAXmdlbH)+3*(MAXbrickV-MAXmdlbV)+12*MAXbrickQ)
!
      ...
      FullDPG(k4+1:k5,k3+1:k4) =  FullDPG(k4+1:k5,k3+1:k4)                        &
                               +  FOSLS_DefGrad(1:ndofphysics(5),1:ndofphysics(4))

      FullDPG(k4+1:k5,k4+1:k5) =  FullDPG(k4+1:k5,k4+1:k5)                        &
                               +  FOSLS_DefGrad(1:ndofphysics(5),ndofphysics(4)+1:ndofphysics(4)+ndofphysics(5))
...
!
      if (iprint.eq.1) then
        write(*,*) 'EnrStiffness = '
        do i=1,enrdof
          write(*,6001) i,EnrStiffnessT(1:kmin,i)
        enddo
...
! 
      deallocate(FullDPG)
!
end subroutine elem_DPG_UW_general
! 