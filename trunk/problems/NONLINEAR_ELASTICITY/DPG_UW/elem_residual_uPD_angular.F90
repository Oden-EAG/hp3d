...
!  ...miscellaneous
      ...
      real*8  :: weight,wa,rjac,bjac,tmp,diff,ptmp(3,3)
!
!  ...LAPACK stuff
      character uplo
      integer :: nk
...
! 
!    P A R T  4 : angular momentum balance as a FOSLS implementation
!
!  FOSLS_Resid = || P(I+D)^T - (I+D)P^T ||^2
!
        ptmp = matmul(PK1,transpose(DEL+Gradu))
        ptmp = ptmp - transpose(ptmp)
        FOSLS_Resid = FOSLS_Resid + 2.d0                                &
                    * (ptmp(1,2)**2.d0+ptmp(1,3)**2.d0+ptmp(2,3)**2.d0) &
                    * weight
