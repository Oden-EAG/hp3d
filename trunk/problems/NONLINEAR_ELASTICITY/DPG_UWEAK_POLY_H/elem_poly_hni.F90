...
!     stiffnes and load matrices for the enriched test space
  ...
  real*8, dimension(2*MAXPP+1,(3*MAXtetraVV+3*MAXtetraHH))   :: EnrLoad_tmp
  real*8, dimension(2*MAXPP+1,2*MAXPP+1)                     :: HNI_Lambda
  real*8, dimension(2*MAXPP+1)                               :: lambda
  integer,dimension(2*MAXPP+1)                               :: IPIV
...
! !  ...initialize the Gram matrix
      Gram=ZERO
!
      ! write(*,*) 'Number of integration points (HNI), nordp =',nint,nordp
      ! clock1=omp_get_wtime()
! xi = 0.d0
! call shap3VV_mono(xi,nordP, nrdofVV,shapVV,divVV,degV)
! !-----------------------------------------------------------------------------------
! !-----------------------------------------------------------------------------------
! !   E L E M E N T   I N T E G R A L    O F    L O A D    T H R O U G H    H N I    |
! !-----------------------------------------------------------------------------------
! !-----------------------------------------------------------------------------------
! !
! ! 
! ! ....Initialize lambdas and compute entries for HNI matrix
!       lambda = 0.d0
!       do i=1,2*nordP+1
!         lambda(i)=1.d0-0.04d0*real(i-1,8)!real(i+8,8)/real(i+9,8) !1.d0-0.04d0*real(i-1,8)
!       enddo
!       HNI_Lambda = 0.d0
!       do j=1,2*nordP+1
!         do i=1,2*nordP+1
!           HNI_Lambda(i,j) = real(3+j-1,8)*lambda(i)**(j-1)
!         enddo
!       enddo
! ! ... Initialize RHS
!       EnrLoad_tmp = ZERO
! ! 
! ! ... Apply quadrature Q^\lambda to (f,v)
! ! 
...
! !    ... loop over lambdas
!         do i=1,nordP+1
! !     ... Evaluate H1 (test) at xi
!           call shap3HH_mono(xi,nordP, nrdofHH,shapHH,gradHH,degH)  
! !     ... get i^th quadrature point in quadrature Q^\lambda
!           xi(1:3) = lambda(i)*xiloc(1:3,ipt)
! !     ... transform xi to physical coordinates (to evaluate f)
!           call poly_affine_3d_to_x(xi,1,a_aff,x)
! !     ... get the source term
!           call getf(Mdle,x, fval)
! !     ... OUTER loop through enriched H1 dofs
!           do k1=1,nrdofHH
...
! !   f \cdot v
! !
!               EnrLoad_tmp(i,m1) = EnrLoad_tmp(i,m1)                  &
...
!       IPIV = 0
! !     set relevant dimensions for DGESV
!       n = 2*nordP+1
!       m = 2*MAXPP+1
!       k = 3*nrdofHH
! !     solve HNI_Lambda * X = EnrLoad_tmp. Result will be returned in EnrLoad_tmp
!       call DGESV(n,k,HNI_Lambda(:,1:n),m,IPIV(1:n),EnrLoad_tmp(:,1:k),m,info)
! !     check for failed solution
!       if (info.ne.0) then
!         write(*,*) 'elem_poly_hni: DGESV did not succeed. mdle, info=',mdle,info
!         stop
!       endif
! !
! !     For each test function, sum over homogeneous components and store in actual Load vector
! !
...
! !     ....counter of row
!           m1 = (k1-1)*3+jcomp
!           EnrLoad(3*nrdofVV+m1,1) = sum(EnrLoad_tmp(1:n,m1),1)
!         enddo
!       enddo
! ! 
! ! 
              ...
              do icomp=1,3
                ...
                if (icomp.eq.jcomp) then
                  EnrFieldDispl(m1,m4) = EnrFieldDispl(m1,m4)                 &
                                       + shapQ(k4)*divVV(k1)                  &
                                       / real(3+degV(k1)-1+degQ(k4),8)*weight
                endif
              enddo
            enddo
!
                ...
                if (icomp.eq.1) then
                  if (jcomp.eq.2) then
                    EnrFieldOmega(m1,m5) = EnrFieldOmega(m1,m5)               &
                                         + shapQ(k5)*shapVV(3,k1)             &
                                         / real(3+degV(k1)+degQ(k5),8)*weight
                  elseif (jcomp.eq.3) then
                    EnrFieldOmega(m1,m5) = EnrFieldOmega(m1,m5)               &
                                         - shapQ(k5)*shapVV(2,k1)             &
                                         / real(3+degV(k1)+degQ(k5),8)*weight
                  endif
                elseif (icomp.eq.2) then
                  if (jcomp.eq.1) then
...
! !   f \cdot v
! !
! !!!!!!!!!!!!!!!!!!!! I set nordP as homogeneous oder for fval, but this is a delicate point
!             EnrLoad(m1,1:NR_RHS) = EnrLoad(m1,1:NR_RHS)         &
!                            + fval(jcomp,1:NR_RHS)*shapHH(k1)    &
!                            /real(3+degH(k1)+nordP,8)*weight
!
!            G R A M   M A T R I X
!
            select case(TEST_NORM)
            ...
            do k3=1,nrdofQ
              do icomp=1,6
                ...
                if (icomp.eq.1) then
                  if (jcomp.eq.1) then
                    EnrFieldStress(m1,m3) = EnrFieldStress(m1,m3)               &
                                          + shapQ(k3)*gradHH(1,k1)              &
                                          /real(3+degH(k1)-1+degQ(k3),8)*weight
                  endif
                elseif (icomp.eq.2) then
                  if (jcomp.eq.2) then
                    EnrFieldStress(m1,m3) = EnrFieldStress(m1,m3)               &
                                          + shapQ(k3)*gradHH(2,k1)              &
                                          /real(3+degH(k1)-1+degQ(k3),8)*weight
                  endif
                elseif (icomp.eq.3) then
                  if (jcomp.eq.3) then
                    EnrFieldStress(m1,m3) = EnrFieldStress(m1,m3)               &
                                          + shapQ(k3)*gradHH(3,k1)              &
                                          /real(3+degH(k1)-1+degQ(k3),8)*weight
                  endif
                elseif (icomp.eq.4) then
                  if (jcomp.eq.1) then
...
! 
!
      ! clock2=omp_get_wtime()
      ! write(*,*) 'mdle, nordp=',mdle,nordp
      ! write(*,*) 'ELEMENT INTEGRATION TIME (HNI)=',clock2-clock1

!-----------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------
!    L O A D    I N T E G R A L    T H R O U G H    T R I A N G U L A T I O N      |
!-----------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------
!
...
!
      ...
      ! clock2=omp_get_wtime()
      ! write(*,*) 'mdle, nordp=',mdle,nordp
      ! write(*,*) 'ELEMENT INTEGRATION TIME (HNI including LOAD VECTOR)=',clock2-clock1
!
!-----------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------
...
! get quadrature points for polygon using the affine coordinates
        INTEGRATION = max(0, (nord_add_local+1)/2)
        call set_quadrature_affine_face_DPG(mdlf,NODES(Mdlf)%order,nrv_f,xverts_f,b_aff,nint,tloc,wt)
        INTEGRATION = 0
...
!
 ...
 !        gdump=75
 !        open(unit=gdump,file='output/gram_mono', &
 ...
 !        close(gdump)
 !        bdump1=76
 !        open(unit=bdump1,file='output/b_u_hni', &
 !          form='formatted',access='sequential',status='unknown')
 !        do i=1,enrdof
 ...
 !        close(bdump1)
 !        bdump2=77
 !        open(unit=bdump2,file='output/b_sigma_hni', &
 !          form='formatted',access='sequential',status='unknown')
 !        do i=1,enrdof
 ...
 !        close(bdump2)
 !        bdump3=78
 !        open(unit=bdump3,file='output/b_omega_hni', &
 !          form='formatted',access='sequential',status='unknown')
 !        do i=1,enrdof
 ...
 !        close(bdump5)
 !        bdump6=81
 !        open(unit=bdump6,file='output/f_hni', &
 !          form='formatted',access='sequential',status='unknown')
 !        do i=1,enrdof
...
! ) 
 ...
 !        gdump=75
 !        open(unit=gdump,file='output/gram_hni', &
 !          form='formatted',access='sequential',status='unknown')
 !        do i=1,enrdof
 !          write(gdump,5999)Gram((i+1)*i/2-(i-1):(i+1)*i/2)
 ! 5999     format(528(e22.15,","))
...
!       EnrFieldOmegac=EnrFieldOmega
      ...
      do i=1,enrdof
        EnrEverything(i,:) = diag_pc(i)*EnrEverything(i,:)
      enddo
...
!  ...G^-1 * EnrEverything
      ...
      do i=1,enrdof
        EnrEverything(i,:) = EnrEverything(i,:)*diag_pc(i)
      enddo
