...
!---------------------------------------------------------------------------------------
  real*8 :: u(3),trac(3),F(3,3),fJ,fJx, &
            ...
            QR(3,3),Strs(3),press, PK1(3,3), w, d2w(3,3,3,3)
  integer :: m , imat
!---------------------------------------------------------------------------------------
!
...
! for the correct storage of stress as Hdiv rows, we need the transpose
    ...
    PK1 = transpose(valV(:,4:6))!reshape(ValQ(NRQVAR+4 :NRQVAR+12),(/3,3/) )

    F = reshape(ValQ(NRQVAR+4:NRQVAR+12),(/3,3/)) + DEL
...
! for the correct storage of stress as Hdiv rows, we need the transpose
    ...
    ! PK1 = transpose(solV(:,4:6))!reshape(solQ(NRQVAR+4 :NRQVAR+12),(/3,3/) )

    F = reshape(solQ(NRQVAR+4:NRQVAR+12),(/3,3/)) + DEL
    call get_det_mat(F,fJ)


    call find_material(Mdle,imat)

    call eval_strain_energy_w_f(imat,X,F,w,PK1,d2w)
