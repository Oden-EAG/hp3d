...
!==============================================================================
  case(0)
!
! This is usually zero or constant. Don't forget to multiply by the load factor
  Fval = Fval * LOAD_FACTOR
...
! 
!   compute exact soution
    call hyperelast_solution(Mdle,X, u,gradu,epsilon,sigma,divsigma)
!
    select case(Ibc)
!   NEUMANN BC
    case(2)
...
!       g_i = sigma_ij n_j
        do j=1,3
          Gval(1:3,iload) = Gval(1:3,iload) + sigma(1:3,j)*Rn(j) *LOAD_FACTOR
        enddo
      enddo
!   INTERFACE BC
    case(3)
