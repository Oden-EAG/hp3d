...
!-------------------------------------------------------------------------------------------
!
subroutine my_paraview_driver(IParAttr,Icustom)
!
   use upscale
...
!
   ...
   logical, intent(in),optional :: Icustom(NR_DISP)
!
   real(8) :: time
...
! 
!           ...if the field is displacement, we write it as a vector to be able to warp the geometry
               if (PHYSA(iphys).eq.'Displ'.and.icomp.eq.1) call paraview_attr_vector(id,idx)
!
            endif
...
!..end loop over rhs's
   ...
   if (.not.PRESENT(Icustom)) goto 90
   ! set approximate solution for evaluation by soldis
   IEXACT_DISP = 2
   do icomp = 1,NR_DISP
      if (Icustom(icomp)) then
         ! set display variable
         ICHOOSE_DISP = icomp
         call paraview_attr_custom(id,icomp)
      endif
   enddo
