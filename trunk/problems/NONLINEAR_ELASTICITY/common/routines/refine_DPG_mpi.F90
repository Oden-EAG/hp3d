...
!$OMP REDUCTION(MAX:elem_resid_max)
 ...
 ! 
      ...
      if (NEXACT.ge.1) then
         call element_error_modif(mdle,Nflag, errorH,errorE,errorV,errorQ,  &
         ...
         select case(PhysNick)
            ...
            case(1010)
               error_subd = error_subd + errorH + errorV
               rnorm_subd = rnorm_subd + rnormH + rnormV
            case default
...
!-----------------------------------------------------------------------
subroutine href_solve(Iref,Nsteps,Nflag,PhysNick, Nstop)
!
   use common_prob_data
   use control
   use data_structure3D
...
!
   implicit none
!
   integer, intent(in)  :: Iref,Nsteps,Nflag(NR_PHYSA)
   integer, intent(in)  :: PhysNick
   integer, intent(out) :: Nstop
!
   integer :: count,src,ierr,i
   logical :: ires
!
   Nstop = 0
   ires = .true.
!
 ...
 !         case default
 !            write(*,*) 'href_solve: refinement type not supported.'; stop
 !         end select
 !         read(*,*) nsteps
 !      enddo
 !   else
