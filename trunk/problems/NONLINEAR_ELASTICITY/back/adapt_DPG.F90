...
!                        > 0 the problem has already been solved
!             Istep      - step number
...
!
      ...
      dimension nrdof_mesh(max_step),residual_mesh(max_step),  &
                rate_mesh(max_step),                           &
                error_H1_mesh(max_step),                    &
                rel_error_H1_mesh(max_step),                &
                ratio_H1vsEN_mesh(max_step),nflag(NR_PHYSA)
!
...
!
      ...
      errorH1 = 0.d0; rnormH1 = 0.d0
      nrdof_total = NRDOFSH+NRDOFSV
      mdle=0
      do iel=1,NRELES
...
!  ...print out the history of refinements
 ...
 7003     format('nrdof = ',i6,' residual = ',e12.5,' rate = ',f7.2,  &
          ...
          error_H1_mesh(i),rel_error_H1_mesh(i),  &
          ratio_H1vsEN_mesh(i),i
 7004     format('nrdof = ',i6,' residual = ',e12.5,' rate = ',f7.2,  &
                 ' H1 error = ',e12.5,  &
                 ' relative H1 error = ',e12.5,  &
