...
if intvol
    ...
    h_threshold=hmin_intvol+0.75*(hmax_intvol-hmin_intvol);
    for j=1:intvol_n
        el = el + 1;
        new_intvol(j,1:3)=seeds_intvol(j,1:3);
        new_intvol(j,4)=h_K_intvol(j)*max(0.75,1-elem_ref_flag(el,3)^0.5);
%         if elem_ref_flag(el,2)==1
...
h_threshold=hmin_surf+0.75*(hmax_surf-hmin_surf);
