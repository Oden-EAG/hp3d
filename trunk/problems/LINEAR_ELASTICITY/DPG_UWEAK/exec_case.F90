...
!
   ...
   integer :: physNick, flag(5), nstop , nsteps
!
!----------------------------------------------------------------------
!
   physNick = 0001
   flag=0; flag(3:4)=1; flag(5)=1;
! 
   solved = .false.
!
   select case(idec)
...
!  ...Multi-step uniform h refinement
      case(22)
         ...
         write(*,*) 'uniform h-refinements...'
         call href_solve(IUNIFORM,nsteps,flag,physNick, nstop)
!
...
!
      case(26)
         ...
         call close_mesh; if (RANK.eq.ROOT) call pause; call MPI_BARRIER (MPI_COMM_WORLD, ierr)
         call update_gdof; if (RANK.eq.ROOT) call pause; call MPI_BARRIER (MPI_COMM_WORLD, ierr)
         call update_Ddof; if (RANK.eq.ROOT) call pause; call MPI_BARRIER (MPI_COMM_WORLD, ierr)
! 
!  ...multiple adaptive h-refinements
      case(27)
         if (RANK .eq. ROOT) then
            nsteps=0
            do while (nsteps.le.0)
         ...
         write(*,*) 'adaptive h-refinements...'
         call href_solve(IADAPTIVE,nsteps,flag,physNick,nstop)
!
!  ...distribute mesh
      case(30)
