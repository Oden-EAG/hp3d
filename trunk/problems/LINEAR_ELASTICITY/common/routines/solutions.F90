...
! local variables
  ...
  real*8, dimension(3,3)     :: TmpTensor,    transf
...
!     Sigma = - pres * del + 2.d0 * mu * Epsilon

!     DivSigma = - grad_pres !+ 2.d0 * mu * Grad2U
...
!
! Cubic solution with vanishing values at vertices and edges of triangle
! (0,0,0) (1,0,0) (1,0,1)
  case(30)
!   displacement
    U(1) = x1**2*x3 + x3**2 - x1*x3**2 - x1*x3
!
!   1st order derivatives
    GradU(1,1) = 2.d0*x1*x3 - x3**2 - x3
    GradU(1,3) = x1**2 + 2.d0*x3 - 2.d0*x1*x3 - x1
!
!   2nd order derivatives
    Grad2U(1,1,1) = 2.d0*x3
    Grad2U(1,1,3) = 2.d0*x1 - 2.d0*x3 - 1.d0
    Grad2U(1,3,1) = 2.d0*x1 - 2.d0*x3 - 1.d0
    Grad2U(1,3,3) = 2.d0 - 2.d0*x1
