...
subroutine exact_error_element(Mdle, Derr,Dnorm)
  ...
  ! local error
  real*8 :: derr_l2, derr_h1(3), derr_en
  ...
  !
  !  miscellaneous
  integer :: i,k,l,nint,icase,icomp,iflag,icompH,iload
...
7001 format('exact_error_element: Mdle,type = ',i10,2x,a5)
  ...
  derr_l2 = 0.d0; derr_h1 = 0.d0; derr_en = 0.d0
  ...
  do l=1,nint
    ...
    case(IERROR_ENERGY)
      ...
      ! compute J(u_h)
      iload=1
      call energy_functional(Mdle,x,zsolH,zdsolH,iload, jsol)

      Dnorm   = Dnorm  +1.d0*weight
      derr_en = derr_en+jsol*weight
...
!-----------------------------------------------------------------
!> Purpose : compute energy functional for PVW linear elasticity
!!
!! @param[in]  Mdle   - element (middle node) number
...
!-----------------------------------------------------------------
subroutine energy_functional(Mdle,X,ZsolH,ZdsolH,Iload, Jsol)
  use isotropic_elast_material
...
! compute J
  Jsol=0.d0
! b(u,v) = EE_ijkl du_k/dx_l dv_i/dx_j
! l(u)   = F_i u_i
! J(u)   = 1/2 b(u,u)-l(u)
  do i=1,3
    do j=1,3; do k=1,3; do l=1,3
      Jsol = Jsol + 0.5d0*EE(i,j,k,l)*ZdsolH(k,l)*ZdsolH(i,j)
    enddo; enddo; enddo
    Jsol = Jsol - zfval(i,Iload)*ZsolH(i)
  enddo

end subroutine energy_functional
