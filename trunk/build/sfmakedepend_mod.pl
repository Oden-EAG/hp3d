#!/usr/bin/env perl
#
# This file is customized for the hp3d library makefile. 
# POC. kyungjoo@utexas.edu
#
# svn $Id: sfmakedepend 1163 2010-02-19 00:28:48Z kate $
#######################################################################
...
my ($opt_help, $opt_man, $opt_file, @incdirs, @srcdirs, @defines,
    $opt_modext, $opt_case, $compiler, $opt_depend, $drop_circ, $srcprefix);
...
if ( !( $cray || $parasoft) ) {
    ...
    foreach $modname (sort keys(%main::mod_files)) {
        ...
        my $object;
        if( $suffix eq ".c" ) {
            $object = $path . $name . "." . "c.o";
        } elsif ( $suffix eq ".F90" ) {
            $object = $path . $name . "." . "f90.o";
        } elsif ( $suffix eq ".F" ) {
            $object = $path . $name . "." . "f.o";
        }
        if (!( $drop_circ && lc($modname) eq lc($name)) ) {
            ...
            print "$modfile: $objfile $srcprefix$path$name$suffix\n";
            print "\t \@if [ ! -f \$\@ ]; then rm \$\< ; \$(MAKE) \$\< -j\$(J) ; fi \n";
        }
    }
}
...
sub print {
    ...
    my($len_sum, $len);
    my($base,$path,$suffix, $object, $modname, $flag, $target, $ftarget);

    ($base,$path,$suffix) = main::fileparse($source, @main::suffixes);
    $path=~ s/$srcprefix//;
    if( $suffix eq ".c" ) {
        $target =  $main::obj_dir . $path . $base . "." . "c.o";
    } elsif ( $suffix eq ".F90" ) {
        $target =  $main::obj_dir . $path . $base . "." . "f90.o";
    } elsif ( $suffix eq ".F" ) {
        $target =  $main::obj_dir . $path . $base . "." . "f.o";
    }
    if ($main::cpp) {
