...
!------------------------------------------------------------------------------
  ...
  real*8,dimension(NR_RHS),intent(out) :: Zfval
!------------------------------------------------------------------------------
! exact solution
...
!------------------------------------------------------------------------------
  ...
  real*8,dimension(NR_RHS),intent(out) :: Zgval
!------------------------------------------------------------------------------
! exact solution
  real*8,dimension(  MAXEQNH    ) ::   zvalH
  real*8,dimension(  MAXEQNH,3  ) ::  zdvalH
...
! Y O U R   F A V O R I T E   H1   S O L U T I O N   H E R E                  !
     zfx   =  sin(X(1)); zfy   =  cos(X(2)); zfz  = X(3)
     zdfx  =  cos(X(1)); zdfy  = -sin(X(2)); zdfz = 1.d0
     zd2fx = -sin(X(1)); zd2fy = -cos(X(2)); zdfz = 0.d0

     ! solution                                                                           
     ZvalH(1)    = zfx *zfy *zfz

     ! gradient                                                                           
     ZdvalH(1,1) = zdfx*zfy *zfz
     ZdvalH(1,2) = zfx *zdfy*zfz
     ZdvalH(1,3) = zfx *zfy *zdfz

     ! hessian                                                                            
     Zd2valH(1,1,1) = zd2fx*zfy  *zfz
     Zd2valH(1,2,2) = zfx  *zd2fy*zfz
     Zd2valH(1,3,3) = zfx  *zfy  *zd2fz

     Zd2valH(1,1,2) = zdfx*zdfy*zfz
     Zd2valH(1,1,3) = zdfx*zfy *zdfz
     Zd2valH(1,2,3) = zfx *zdfy*zdfz

     Zd2valH(1,2,1) = zdfx*zdfy*zfz
     Zd2valH(1,3,1) = zdfx*zfy *zdfz
     Zd2valH(1,3,2) = zfx *zdfy*zdfz
