...
!------------------------------------------------------------------------------
  ...
  VTYPE,dimension(3,NR_RHS),intent(out) :: Zfval
!------------------------------------------------------------------------------
! exact solution
...
! 
!------------------------------------------------------------------------------
!> Purpose : impressed surface current
!!
!! @param[in]  Mdle  - element (middle node) number
...
!------------------------------------------------------------------------------
  ...
  VTYPE,dimension(3,NR_RHS),intent(out) :: Zgval
!------------------------------------------------------------------------------
! exact solution
  VTYPE,dimension(  MAXEQNH    ) ::   zvalH
  VTYPE,dimension(  MAXEQNH,3  ) ::  zdvalH
...
!   Y O U R   F A V O R I T E   N E U M A N N   L O A D   H E R E             !
    Zgval(1,1:NR_RHS) = (/1.d0,0.d0/)
    Zgval(2,1:NR_RHS) = (/1.d0,0.d0/)
    Zgval(3,1:NR_RHS) = (/1.d0,0.d0/)
!******************************************************************************    
  !
  ! 
  ! known exact solution
